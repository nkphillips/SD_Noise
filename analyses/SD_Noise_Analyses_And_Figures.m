...
for i_n_back = 1:length(n_back_conditions)
        ...
        for n_run = 1:num.runs(subj)
            ...
            for cond = 1:num.conds
                ...
                for n_block = 1:length(curr_cond_blocks)
                    ...
                    total_trials_per_cond(cond, subj) = sum(total_trials_per_cond(cond, subj), 'omitnan') + length(curr_lvls) - n_back; % -n_back because first trial has no previous trial
                end
            end
    ...
    if toggles.disp_on
        ...
        disp(['  - Filtered out ' num2str(num.levels * num.levels * num.conds * num.delta_theta_windows - task_count) ' empty windows']);
        disp(' ');
    end
    ...
    try
        generateReports(trial_counts, total_trials_per_cond, p, num, sd, reportMeta);
    catch reportErr
