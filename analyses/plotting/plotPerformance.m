            ...
            for prev_lvl = 1:num_levels
                for curr_lvl = 1:num_levels
                    ...
                    for i_window = 1:numel(delta_theta_centers)
                        cw_resp = responses_cell{prev_lvl, curr_lvl, cond_tmp, i_window};
                        probe_offs = probe_offsets_cell{prev_lvl, curr_lvl, cond_tmp, i_window};
                        if ~isempty(cw_resp) && ~isempty(probe_offs)
                            ...
                            switch m_idx
                                case 1 % Percent Correct
                                    ...
                                    y_vals_tmp(i_window) = 100 * mean(is_correct, 'omitnan');
                                case 2 % Percent CCW
                                    y_vals_tmp(i_window) = 100 * mean(1 - cw_resp, 'omitnan');
                            end
                        end
                    end
        ...
        for cond = 1:num_conds
            ...
            for prev_lvl = 1:num_levels
                for curr_lvl = 1:num_levels
                    ...
                    for i_window = 1:numel(delta_theta_centers)
                        cw_resp = responses_cell{prev_lvl, curr_lvl, cond, i_window};
                        probe_offs = probe_offsets_cell{prev_lvl, curr_lvl, cond, i_window};
                        if ~isempty(cw_resp) && ~isempty(probe_offs)
                            cw_resp = cw_resp(:);
                            probe_offs = probe_offs(:);
                            n = min(numel(cw_resp), numel(probe_offs));
                            ...
                            switch m_idx
                                case 1 % Percent Correct
                                    is_correct = (cw_resp == (probe_offs > 0));
                                    y_vals(i_window) = 100 * mean(is_correct, 'omitnan');
                                case 2 % Percent CCW
                                    y_vals(i_window) = 100 * mean(1 - cw_resp, 'omitnan');
                            end
                        end
                    end
