...
% Points are horizontally dodged at each n-back to keep bootstrap CIs readable.
    ...
    if isempty(all_y)
        ...
        y_lims = [min(0, y_min - y_pad), y_max + y_pad];
    end
...
function local_renderByFixedAxis(fig_dir, amp, amp_lo, amp_hi, n_back_list, y_lims, ...
    ...
    for cond = 1:num_conds
        ...
        for fixed_level = 1:num_levels
            ...
            for line_level = 1:num_levels
                ...
                y = squeeze(amp(prev, curr, cond, :))';
                lo = squeeze(amp_lo(prev, curr, cond, :))';
                hi = squeeze(amp_hi(prev, curr, cond, :))';
                ok = isfinite(y);
                if ~any(ok)
                    continue
                end
