function tt = computeUnbinnedAllLevelTrendTests(results, n_back)
% computeUnbinnedAllLevelTrendTests  Bootstrap trend tests over all 3 x 3 cells.
%
...
function metric = local_metricValues(results, parameter)
    switch parameter
        ...
        otherwise
            error('computeUnbinnedAllLevelTrendTests:badParameter', 'Unknown parameter: %s', parameter);
    end
end
...
function idx = local_termIndex(term)
    switch char(term)
        ...
        case 'previous_current_interaction'
            idx = 3;
        otherwise
            error('computeUnbinnedAllLevelTrendTests:badTerm', 'Unknown term: %s', char(term));
    end
end
