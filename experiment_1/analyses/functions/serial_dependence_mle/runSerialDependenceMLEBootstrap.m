function results = runSerialDependenceMLEBootstrap(tbl_trials, varargin)
% runSerialDependenceMLEBootstrap  B-fold subject-cluster bootstrap, trial-level
% serial-dependence MLE per condition (S&S 2022 DoG + Gaussian psychometric + 25% guess rate).
...
%   'fit_method' ('pooled' | 'hierarchical_map') -- condition-level estimator.
%   'map_lambda' (0.5) -- L2 penalty strength for hierarchical_map subject fits.
%   'ci_prctile' ([2.5 97.5])
...
%                                overlay faint subject points on pooled summaries.
    ...
    addParameter(ip, 'fit_method', 'pooled', @(x) ischar(x) || isstring(x));
    addParameter(ip, 'map_lambda', 0.5, @(x) isnumeric(x) && isscalar(x) && isfinite(x));
    ...
    fit_method = lower(char(ip.Results.fit_method));
    map_lambda = ip.Results.map_lambda;
    ...
    fit_opts.map_lambda = map_lambda;

    if ~ismember(fit_method, {'pooled', 'hierarchical_map'})
        error('runSerialDependenceMLEBootstrap:badFitMethod', ...
            'fit_method must be ''pooled'' or ''hierarchical_map''.');
    end

    if ~ismember(bootstrap_unit, {'subject', 'trial'})
        error('runSerialDependenceMLEBootstrap:badBootstrapUnit', ...
            'bootstrap_unit must be ''subject'' or ''trial''.');
    end

    if ~ismember(ci_method, {'bca', 'percentile'})
        error('runSerialDependenceMLEBootstrap:badCIMethod', ...
            'ci_method must be ''bca'' or ''percentile''.');
    end

    if strcmp(bootstrap_unit, 'trial') && strcmp(ci_method, 'bca')
        warning('runSerialDependenceMLEBootstrap:trialBootstrapPercentileOnly', ...
    ...
    if strcmp(bootstrap_unit, 'trial') && compute_jackknife
        warning('runSerialDependenceMLEBootstrap:noJackknifeForTrialBootstrap', ...
    ...
    if isempty(fig_subdir)
        ...
        fig_dir = fullfile(analysis_dir, 'figures', fig_subdir);
    end
    ...
    if isempty(raw_subject_fig_subdir)
        ...
        raw_subj_dir = fullfile(analysis_dir, 'figures', raw_subject_fig_subdir, 'subject');
    ...
    if ip.Results.demean_x_probe
        try
            ...
            warning('runSerialDependenceMLEBootstrap:demeanFailed', ...
    ...
    if ip.Results.subject_data_quality && ip.Results.write_raw_subject_diagnostics
        try
            ...
            warning('runSerialDependenceMLEBootstrap:rawPerformanceFailed', ...
    ...
    if ip.Results.make_figures && ip.Results.subject_data_quality && ip.Results.write_raw_subject_diagnostics
        try
            ...
            warning('runSerialDependenceMLEBootstrap:dataQualityFailed', ...
    ...
    if n_subj < 1
        error('runSerialDependenceMLEBootstrap:noSubjects', 'tbl_trials has no subjects.');
    end
    ...
    if use_parallel && B > 1
        ...
        if strcmp(bootstrap_unit, 'subject')
            parfor b = 1:B
                ...
                [pr, cr, er, ar] = bootstrapOneReplicate(tbl_trials, plan_row, grid, fit_opts, lb_used, ub_used, bound_tol, bootstrap_unit, fit_method);
    ...
    overlay = computePooledFitsAndOverlay(tbl_trials, delta_centers_emp, empirical_window_width, guess_rate, fit_opts, fit_method);
    ...
    if compute_jackknife
        ...
        jk = runSubjectJackknife(tbl_trials, subj_list, grid, fit_opts, jk_use_parallel, lb_used, ub_used, bound_tol, fit_method);
        jk.duration_sec = toc(t_jk);
    elseif strcmpi(ci_method, 'bca')
        warning('runSerialDependenceMLEBootstrap:noJackknife', ...
    ...
    for c = 1:num_conds
        ...
        beta_vals = reshape(params_boot(:, c, 4), [B, 1]);
        % full mu curve per replicate. Keep B x n_grid shape even when B = 1.
        full_curve = reshape(curve_boot(:, c, :), [B, n_grid]) + beta_vals;
        ...
        if all(isfinite(pp(c, :)))
            jk_iso = reshape(jk.curves(:, c, :), [n_subj, n_grid]);
            jk_beta = reshape(jk.params(:, c, 4), [n_subj, 1]);
    ...
    if ip.Results.close_far_sigma
        try
            ...
            warning('runSerialDependenceMLEBootstrap:closeFarSigmaFailed', ...
    ...
    results.fit_method       = fit_method;
    results.map_lambda       = map_lambda;
    ...
    if ip.Results.subject_cell_points
        try
            ...
            warning('runSerialDependenceMLEBootstrap:subjectCellFitsFailed', ...
    ...
    if ip.Results.compute_contrasts
        ...
        try
            ...
            warning('runSerialDependenceMLEBootstrap:contrastFailed', ...
    ...
    try
        ...
        warning('runSerialDependenceMLEBootstrap:allLevelTrendFailed', ...
    ...
    try
        ...
        warning('runSerialDependenceMLEBootstrap:simpleSlopeTrendFailed', ...
    ...
    if ip.Results.subject_influence
        if ~compute_jackknife
            warning('runSerialDependenceMLEBootstrap:noJackknifeForInfluence', ...
            ...
            try
                ...
                warning('runSerialDependenceMLEBootstrap:subjectInfluenceFailed', ...
    ...
    if ip.Results.amplitude_sigma_correlation
        try
            ...
            warning('runSerialDependenceMLEBootstrap:amplitudeSigmaFailed', ...
...
function jk = runSubjectJackknife(tbl_trials, subj_list, grid, fit_opts, use_parallel, lb, ub, bound_tol, fit_method)
    n_subj = numel(subj_list);
    n_grid = numel(grid);
    num_conds = 18;
    ...
    if use_parallel
        parfor k = 1:n_subj
            [sp, cc, ab] = jackknifeOneSubject(k, subj_list, tbl_trials, grid, fit_opts, lb, ub, bound_tol, fit_method);
            sd_p_cell{k}  = sp;
            curve_cell{k} = cc;
            bound_cell{k} = ab;
        end
...
function [sd_params, curves, at_bound] = jackknifeOneSubject(k, subj_list, tbl_trials, grid, fit_opts, lb, ub, bound_tol, fit_method)
    num_conds = 18;
    n_grid = numel(grid);
    ...
    for c = 1:num_conds
        ...
        tbl_cond = tbl_jk(mask, {'subject_id', 'delta_theta', 'x_probe', 'response'});
        pf = fitConditionByMethod(tbl_cond, fit_opts, fit_method);
        sd_params(c, :) = pf;
        if ~all(isnan(pf))
