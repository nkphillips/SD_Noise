%%% Run_Serial_Dependence_MLE_Bootstrap
%
% Sole entry point for the trial-level serial-dependence DoG + psychometric MLE with
...
%
% Outputs: PDFs under figures/<fit_method>/<analysis_datetime>/; per-n-back
% estimate snapshots plus a canonical sd_noise struct under
% estimates/<fit_method>/<analysis_datetime>/; workspace
...
bootstrap.fit_method = 'pooled';        % 'pooled' | 'hierarchical_map'
bootstrap.map_lambda = 0.5;             % L2 regularization strength for hierarchical_map
...
if regenerate_figures_only
    ...
    if isempty(sd_noise_file)
        estimates_root = fullfile('estimates', char(bootstrap.fit_method));
        if isempty(regenerate_analysis_datetime)
            ...
            files = dir(fullfile(estimates_root, regenerate_analysis_datetime, ...
                'SD_Noise_SerialDependence_sd_noise_*.mat'));
        end
        if isempty(files)
            error('Run_Serial_Dependence_MLE_Bootstrap:noSavedAnalysis', ...
    ...
    if ~isfield(S, 'sd_noise')
        error('Run_Serial_Dependence_MLE_Bootstrap:missingSdNoise', ...
...
%% Run serial-dependence MLE pipeline per n_back
% Single source of truth: one nv block, one loop. When n_back_list is a single
% value, `results` is the bare results struct from runSerialDependenceMLEBootstrap.
...
sd_noise.meta = struct( ...
    'pipeline', 'serial_dependence_mle', ...
...
sd_noise.paths = struct( ...
    'analysis_dir', pwd, ...
    'figure_root', fullfile('figures', char(bootstrap.fit_method), analysis_datetime), ...
    'estimate_root', fullfile('estimates', char(bootstrap.fit_method), analysis_datetime));
...
for i_nb = 1:numel(n_back_list)
    ...
    disp(' ');
    disp(['=== Serial-dependence MLE bootstrap | n_back = ' num2str(n_back) ' ===']);
    ...
    nv = { ...
        ...
        'fit_method', bootstrap.fit_method, ...
        'map_lambda', bootstrap.map_lambda, ...
        ...
        'raw_subject_fig_subdir', fullfile(char(bootstrap.fit_method), analysis_datetime), ...
        'n_back', n_back, ...
        ...
        'fig_subdir', fullfile(char(bootstrap.fit_method), analysis_datetime, sprintf('%d_back', n_back)) ...
        };
    ...
    res_i = runSerialDependenceMLEBootstrap(tbl_trials, nv{:});

    result_key = sprintf('n%d', n_back);
    est_dir = fullfile('estimates', char(bootstrap.fit_method), ...
        analysis_datetime, sprintf('%d_back', n_back));
    if ~exist(est_dir, 'dir')
        mkdir(est_dir);
    end
    est_fname = sprintf('SD_Noise_SerialDependence_Estimates_%s_%s.mat', ...
    ...
    fprintf('Saved serial-dependence estimates: %s\n', fullfile(est_dir, est_fname));
    ...
    if toggles.fold_delta_theta_fit
        disp(' ');
        disp(['=== Folded delta-theta serial-dependence MLE | n_back = ' num2str(n_back) ' ===']);
        ...
        folded_fig_subdir = fullfile(char(bootstrap.fit_method), analysis_datetime, sprintf('%d_back', n_back), 'folded_delta_theta');
        nv_folded = nv;
        ...
        res_folded = runSerialDependenceMLEBootstrap(tbl_trials_folded, nv_folded{:});
        res_folded.fold_info = fold_info;

        result_key_folded = sprintf('n%d_folded_delta_theta', n_back);
        est_dir_folded = fullfile('estimates', char(bootstrap.fit_method), ...
            analysis_datetime, sprintf('%d_back', n_back), 'folded_delta_theta');
        if ~exist(est_dir_folded, 'dir')
            mkdir(est_dir_folded);
        end
        est_fname_folded = sprintf('SD_Noise_SerialDependence_Estimates_%s_%s_folded_delta_theta.mat', ...
            analysis_datetime, bootstrap.ci_method);
        ...
        fprintf('Saved folded serial-dependence estimates: %s\n', fullfile(est_dir_folded, est_fname_folded));
...
sd_noise_path = fullfile(sd_noise.paths.estimate_root, ...
    sprintf('SD_Noise_SerialDependence_sd_noise_%s_%s.mat', analysis_datetime, bootstrap.ci_method));
...
disp(['Finished. Figures under ' fullfile('figures', char(bootstrap.fit_method)) '; estimates under ' fullfile('estimates', char(bootstrap.fit_method)) ' (timestamped).']);
