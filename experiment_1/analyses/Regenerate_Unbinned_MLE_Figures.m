%%% Regenerate_Unbinned_MLE_Figures
%
% Re-render unbinned MLE figures from a saved sd_noise analysis object.
...
if isempty(sd_noise_file)
    ...
    if isempty(analysis_datetime)
        ...
        files = dir(fullfile(estimates_root, analysis_datetime, 'SD_Noise_Unbinned_sd_noise_*.mat'));
    end
    if isempty(files)
        error('Regenerate_Unbinned_MLE_Figures:noSavedAnalysis', ...
...
if ~isfield(S, 'sd_noise')
    error('Regenerate_Unbinned_MLE_Figures:missingSdNoise', ...
