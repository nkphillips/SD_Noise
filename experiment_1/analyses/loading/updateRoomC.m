% updateRoomC
% For each subject, update run_info.p.which_setup to a value of 2 from 1
%   - Load each subject's run_info
%       - hints: dir(), load(), save()
%   - Directly update run_info.p.which_setup
...
%%% updateRoomC
% For each subject, update run_info.p.which_setup to 2
...
for subj = 1:numel(p.subj_IDs)
    ...
    for n_file = 1:numel(data_file_names)
        ...
        run_info.p.which_setup = 2;   % Update which_setup
