...
if staircases.responses(curr_sc, curr_sc_trial, curr_lvl, curr_cond) == 1
    ...
    % Update probe offset if there have been 2 correct responses
    if staircases.correct_count(curr_sc, curr_lvl, curr_cond) == 2
        ...
        if curr_sc_trial < staircases.max_trials_per_sc
            staircases.probe_offsets(curr_sc, curr_sc_trial+1, curr_lvl, curr_cond) = max(staircases.min_probe_offset, curr_probe_offset - curr_step_size);
        end
    ...
    elseif staircases.correct_count(curr_sc, curr_lvl, curr_cond) == 1
        ...
        if curr_sc_trial < staircases.max_trials_per_sc
            staircases.probe_offsets(curr_sc, curr_sc_trial+1, curr_lvl, curr_cond) = curr_probe_offset;
        end
...
% If the direction of the previous trial is different from the current trial
prev_direction = staircases.direction(curr_sc, 1, curr_lvl, curr_cond);
curr_direction = staircases.direction(curr_sc, 2, curr_lvl, curr_cond);
