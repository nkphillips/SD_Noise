...
for subj = 1:num.subjs
    for i_field = 1:numel(fieldname_list)
        ...
        if contains(fieldname_list{i_field},{'performance','sem','pCW'})
            ...
            delta_theta_windows.ind(subj).(fieldname_list{i_field}) = nan(num.levels, num.levels, num.features, num.delta_theta_windows);
            if subj == 1
                delta_theta_windows.all.(fieldname_list{i_field}) = nan(num.levels, num.levels, num.features, num.delta_theta_windows);
                delta_theta_windows.grp.(fieldname_list{i_field}) = nan(num.levels, num.levels, num.features, num.delta_theta_windows);
            end

        elseif contains(fieldname_list{i_field}, {'delta_thetas','probe_offsets','responses','correct_trials'})

            delta_theta_windows.ind(subj).(fieldname_list{i_field}) = cell(num.levels, num.levels, num.features, num.delta_theta_windows);
            if subj == 1
                delta_theta_windows.all.(fieldname_list{i_field}) = cell(num.levels, num.levels, num.features, num.delta_theta_windows);
                delta_theta_windows.grp.(fieldname_list{i_field}) = cell(num.levels, num.levels, num.features, num.delta_theta_windows);
            end
...
all_delta_thetas = cell(num.levels, num.levels, num.features); % Store all delta thetas for each condition for histogram
...
for subj = 1:num.subjs
    ...
    for n_run = 1:num.runs(subj)
        ...
        for cond = 1:num.features
            ...
            curr_cond_blocks = find(subj_p.feature_order == cond);
