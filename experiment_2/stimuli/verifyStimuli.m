...
if toggles.save_texture_figures || toggles.save_energy_figures
    fg = figure('Visible','on','Color',[1 1 1], 'Position', [100 100 800 400]);
...
for fw_idx = 1:n_filter_widths
    ...
    padding = 2^nextpow2(max(size(texture_zero_mean)) * 2);
    
    fft_texture = fftshift(fft2(texture_zero_mean, padding, padding)); 
