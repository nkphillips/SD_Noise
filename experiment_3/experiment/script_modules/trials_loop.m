...
for n_trial = 1:size(p.trial_events,1)
    ...
    for n_frame = 1:frames.test_frames_count
        ...
        if toggles.show_debug_output && n_frame == frames.test_frames_count
            %pause(3);
        end
    ...
    for n_frame = 1:frames.mask_frames_count
        ...
        if toggles.show_debug_output && n_frame == frames.mask_frames_count
            %pause(3);
        end
    ...
    for n_frame = 1:frames.delay_frames_count
        ...
        if toggles.show_debug_output && n_frame == frames.delay_frames_count
            %pause(3);
        end
    ...
    for n_frame = 1:frames.probe_frames_count
        ...
        if toggles.show_debug_output && n_frame == frames.probe_frames_count
            %pause(3);
        end
        ...
        if key_pressed && no_response_recorded
            ...
            if ~isempty(relevant_keys)
                ...
                if first_relevant_key == p.keypress_numbers(1)
                    ...
                    if toggles.show_debug_output, disp('Response: CCW'); end
                elseif first_relevant_key == p.keypress_numbers(2)
                    behav_data.response(n_trial, n_block) = 2;
                    if toggles.show_debug_output, disp('Response: CW'); end
                end
                ...
                if toggles.show_debug_output && behav_data.correct(n_trial, n_block)
                    disp('Correct response!')
                elseif toggles.show_debug_output && ~behav_data.correct(n_trial, n_block)
                    disp('Incorrect response!')
                end

            elseif any(which_press == KbName('ESCAPE'))
                if toggles.show_debug_output, disp('Escape key pressed. Exiting...'); end
                break;
    ...
    if toggles.show_debug_output, disp('  '); end
