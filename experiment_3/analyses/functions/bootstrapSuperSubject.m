...
%   perf_ci.(pc_lo|pc_hi|pccw_lo|pccw_hi): same shape, CIs for performance
    ...
    if nargin < 6
        toggles = struct('show_debug_output', 0);
    end
    ...
    sz = [num.levels, num.levels, num.features, num.delta_theta_windows];
    rb_ci.mu_lo = nan(sz);
    rb_ci.mu_hi = nan(sz);
    ...
    % Optional summary
    if isfield(toggles,'show_debug_output') && toggles.show_debug_output
