...
for subj = 1:length(subj_IDs)
    ...
    for cond = 1:num_conds
        ...
        for lvl = 1:num_levels
            ...
            subplot(num_conds, num_levels, (cond-1)*num_levels + lvl);
            hold on;
            ...
            for n_sc = 1:size(all_staircases(subj).probe_offsets, 1)
                ...
                plot(all_staircases(subj).probe_offsets(n_sc,:,cond,lvl), colors{n_sc}, 'LineWidth', 1);
                
                % Mark reversals
                reversal_idx = find(all_staircases(subj).reversal_indices(n_sc,:,cond,lvl) == 1);
                reversal_values = all_staircases(subj).probe_offsets(n_sc,reversal_idx,cond,lvl);
            ...
            for n_sc = 1:size(all_staircases(subj).probe_offsets, 1)
                ...
                if length(reversal_idx) >= all_staircases(subj).num_reversals_to_consider
                    last_reversals = all_staircases(subj).probe_offsets(n_sc,reversal_idx(end-all_staircases(subj).num_reversals_to_consider+1:end),cond,lvl);
                    cv = std(last_reversals) / mean(last_reversals);
                    mean_threshold = mean(last_reversals);
                    text(0.7, 0.9-0.1*n_sc, sprintf('SC%d: %.1f°', ...
                        n_sc, mean_threshold), ...
                        'Units', 'normalized', 'Color', colors{n_sc});
                end
            end
    ...
    for cond = 1:num_conds
        ...
        for lvl = 1:num_levels
            ...
            plot(1:length(running_mean), 100*running_mean, colors{n_sc}, 'LineWidth', 2);
            
            % Plot individual reversals
            scatter(1:length(reversal_values), 100*reversal_values, 50, colors{n_sc}, 'filled', 'MarkerFaceAlpha', 0.3);
            end
    ...
    for cond = 1:num_conds
        ...
        for lvl = 1:num_levels
            ...
            for n_sc = 1:size(all_staircases(subj).contrast_deltas, 1)
                
                % Get number of trials performed
                num_trials = sum(~isnan(all_staircases(subj).responses(n_sc,:,n_sf)));

                % Find reversal indices
                reversal_idx = find(all_staircases(subj).reversal_indices(n_sc,:,n_sf) == 1);
                ...
                if length(reversal_idx) >= all_staircases(subj).num_reversals_to_consider
                    ...
                    % Calculate mean contrast delta for these reversals
                    mean_delta = mean(all_staircases(subj).contrast_deltas(n_sc, last_reversals_idx, n_sf));
                    mean_deltas(n_sc) = mean_delta;
                    
                    % Find trials near this contrast delta (±1%)
                    delta_window = 1 + mean_delta;
                    deltas = all_staircases(subj).contrast_deltas(n_sc, trial_range, n_sf);
                    responses = all_staircases(subj).responses(n_sc, trial_range, n_sf);
